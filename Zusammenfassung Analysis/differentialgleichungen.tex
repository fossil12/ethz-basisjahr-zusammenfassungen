...
\begin{itemize}
	\item lineare, homogene DGL 1. Ordnung mit konstanten Koeff.
	\item lineare, inhomogene DGL 1. Ordnung mit  konstante Koeff.
	\item nicht lineare, homogene DGL 1. Ordnung mit  konstanten Koeffizienten
	\item lineare, homogene DGL 2. Ordnung mit  konstanten Koeff.
	\item lineare, inhomogene DGL 2. Ordnung mit  konstanten Ko.
	\item In ausnahmenfälle linare DGL höherer Ordnung
\end{itemize}
...
\begin{enumerate}
	\item $ax+b = 0 \Rightarrow$ nach $x$ auflösen
	\item $ax^2 + bx + c = 0 \Rightarrow$ Mitternachtsformel (FoTa)
	\item $\sum_{i=0}^{n} = 0$ mit $n > 2 \Rightarrow$
		\begin{itemize}
			\item Eine Nullstellte durch probieren finden
			\item Polynomdivision
			\item wiederholen oder (2.) anwenden
		\end{itemize}
\end{enumerate}
