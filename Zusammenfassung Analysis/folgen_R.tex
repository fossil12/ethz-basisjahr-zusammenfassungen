...
\begin{description}
  ...
  bzw. $C$ (Schranke) gibt, so dass $|a_n| \leq C$
...
\begin{definition}[Grenzwert] \index{Grenzwert}
	\begin{align*}
		...
		& \Leftrightarrow \forall \epsilon > 0 \ \exists N_0 \in \N \ 
		\forall n \geq N_0: |a_n - a| < \epsilon
	\end{align*}
\end{definition}
...
% DK: Nicht in der Vorlesung gesehen
...
\begin{itemize}[leftmargin=*]	
	...
	\item Ist die Folge \underline{monoton steigend} und nach 
	\underline{oben beschränkt}, dann \underline{existiert} 
	$\lim_{n \to \infty} a_n$. Ist die Folge \underline{monoton fallend} und nach 
	\underline{unten beschränkt}, dann \underline{existiert} 
	$\lim_{n \to \infty} a_n$
	
	\item Konvergiert $\sum_{n=0}^\infty a_n$, so ist $\lim_{n \to \infty} a_n =
	0$\\
	Damit kann man die Regeln für Reihen verwenden. Siehe Grenzwerte von Reihen.
	...
	\item Jede beschränkte Teilfolge besitzt mind. 1 konvergente Teilfolge 
	$\leftrightarrow$ Häufungsfolge
\end{itemize}
