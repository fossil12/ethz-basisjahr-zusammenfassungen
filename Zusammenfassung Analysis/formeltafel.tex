...
%	\int f(x) dx &= \int f(g(t)) \cdot g'(t) dt, \; x=g(t), dx = g'(t) dt \\
%	
%	\int f(a + x) \,dx &= F(a + x)
%	\int f(a - x) \,dx &= -F(a-x)
%	\int f(-x) \,dx &= -F(-x)
%	\int f(\alpha x) \,dx &= \frac{1}{\alpha}F(\alpha x)
%	\int \frac{g'(x)}{g(x)} \, dx &= \ln|g(x)|
%	\int g(x)g'(x) \, dx &= \frac{1}{2}g(x)^2 \\
%	
%	|\int f(x)| \leq \int |f(x)| \text{ (wenn f, Riemann-Integrable ist)}
\end{align*}
