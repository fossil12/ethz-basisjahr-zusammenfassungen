...
%\subsection{Argument} % don't think that this was ever mentioned in the lecture (DK)
%\[
%	\arg(x,y) := \begin{cases}
...
Es gelte: $\int f(x) \, dx = F(x) + C$
\begin{itemize}[leftmargin=*]
	\item $\int f(x)+g(x) \,dx = \int f(x) \,dx + \int g(x) \,dx$
	\item $\int a \cdot f(x) \,dx = a \cdot \int f(x) \,dx$\\
	
	\item $\int u'\cdot v dx = uv - \int u \cdot v' dx$ (partielle Int.)
	\item $\int f(x) dx = \int f(g(t)) \cdot g'(t) \,dt$ mit $x=g(t),\, \frac{dx}{dt} 
		= g'(x), \,dx = g'(t) dt$ (Subsitution)
	\todo{unsicher ob das stimmt} %todo: Unischer, ob Substitution stimmt
	\\
	\item $\int \frac{A}{x-x_1} + \frac{b}{x-x_2} = A \int \frac{1}{x-x_1} +
		B \int \frac{1}{x-x_1} = A(\ln|x-x_1|+C) + B(\ln|x-x_2|+C)$ (nach Subtituion beim letzten Gleich)
	\\

% Weren't useful until now for me (DK)
...
\begin{itemize}[leftmargin=*]
	\item $\int a x^b \,dx = \frac{a}{b+1} x^{b+1}$ ($\int a \,dx = ax$)
	...
	\item $\int \frac{1}{1 + x^2} \,dx = \arctan(x)$ 
			%= \frac{1}{2} \ln |1 + x^2|$ (semms wrong (DK))
	...
	|pq+q|$
	\item $\int \frac{1}{\sqrt{1-x^2}} = \arcsin(x)$
...
\begin{itemize}[leftmargin=*]
	...
	\item $\int \sin^2(ax) \,dx = \frac{x}{2} - \frac{sin(2ax)}{4a}$
	...
	\item $\int_0^\infty \cos(x^2) \,dx = \frac{\sqrt{\frac{\pi}{2}}}{2} \approx 0.620657$
\end{itemize}
...
\begin{itemize}[leftmargin=*]
	\item Volumen x-Achse $= \pi \int_a^b (f(x))^2 \,dx$
	\item (Volumen y-Achse $= 2\pi \int_a^b x \cdot f(x) \,dx$)
	\item Oberfläche = $2 \pi \int_a^b f(x) \sqrt{1 + (f'(x))^2} \,dx$
\end{itemize}

\subsection{Reihen}
\begin{itemize}[leftmargin=*]
	\item $\sum_{n=1}^\infty \frac{1}{n}$ divergiert (``harmonische Reihe'')
	\item $\sum_{n=0}^\infty (-1)^n \frac{1}{n+1} = \ln(2)$ 
...
\begin{itemize}[leftmargin=*]
	\item $\lim_{n \to \infty} \frac{1}{n} \rightarrow 0$
	\item $\lim_{n \to \infty} q^n \rightarrow 0$ für $|q| < 1$
			%ev. Fallunterscheidung
	...
	\item $\lim_{n \to \infty} \arctan{n} \rightarrow \frac{\pi}{2}$
	\item $\lim_{n \to -\infty} \arctan{n} \rightarrow -\frac{\pi}{2}$
...
\begin{itemize}[leftmargin=*]
	...
	\item $\lim_{x \to 0} \frac{\log_a(1+x)}{x} = \frac{1}{\ln a}$
	%\item $\lim_{x \to 0} x^\alpha \ln x = 0 \qquad \alpha > 0$ not used (DK)
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item $a < b$ und $c < 0 \Rightarrow \frac{a}{c} > \frac{b}{c}$ 
  \item $\Delta$-Ungleichung für reelle Zahlen: $|a+b| \le |a|{+}|b|$ %Quelle Wikipedia: http://de.wikipedia.org/wiki/Dreiecksungleichung#Dreiecksungleichung_f.C3.BCr_reelle_Zahlen
